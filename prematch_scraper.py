...
for row in rows:
    ...
    if len(cells) > 0:
        team1 = cells[1].text.split(" ")[-1]
        team2 = cells[3].text.split(" ")[-1]
        Original_Total = float(cells[4].text)
        new_row = {"Team1": team1, "Team2": team2, "Original_Total": Original_Total, "Date": arrow.get(cells[0].text.split(",")[1].strip(), "MMMM D").format('MM-DD')}
